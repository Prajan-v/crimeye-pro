...
import { Shield, AlertCircle, UserPlus } from 'react-feather';
...
theme.colors.background.secondary} 0%, ${({ theme }) => theme.colors.background.primary} 80%);
...
&:active:not(:disabled) { transform: translateY(0); filter: brightness(0.9); box-shadow: ${({ theme }) =>
theme.shadows.sm}; }
...
visible: (i: number) => ({ opacity: 1, x: 0, transition: { delay: i * 0.1, duration: 0.4, ease: 'easeOut' } }),
};
const errorVariants = {
...
// Manually dispatch a rejected action shape for immediate feedback
dispatch({ type: 'auth/registerUser/rejected', payload: 'Password must be at least 6 characters.' });
return;
}
if (!email || !/\S+@\S+\.\S+/.test(email)) {
dispatch(clearAuthError());
// Manually dispatch a rejected action shape
...
navigate('/login'); // Navigate to login even if auto-login fails
}
}
};
return (
...
/>
<motion.input variants={itemVariants} custom={2} type="email" placeholder="Email" value={email}
onChange={(e) => setEmail(e.target.value)} required autoComplete="email" />
<motion.input variants={itemVariants} custom={3} type="password" placeholder="Password (min. 6 characters)" value={password} onChange={(e) => setPassword(e.target.value)} required
autoComplete="new-password" />
<AnimatePresence>
{authError && (
<ErrorMessage key="error-message" variants={errorVariants} initial="hidden" animate="visible"
exit="hidden">
