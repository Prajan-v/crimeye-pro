...
import { RootState } from '../app/store'; // Assuming your store type is exported from here
import { addIncident } from '../features/incidents/incidentsSlice';
import { AiDetection } from '../common/types';

// Define action types for connect/disconnect if needed, or use string literals directly
...
export const socketMiddleware: Middleware<{}, RootState> = store => next => action => {
  const typedAction = action as WsAction; // Type assertion
  ...
  switch (typedAction.type) {
    case 'WS_CONNECT':
      ...
      socket.on('connect_error', (error) => {
        ...
        socket?.close(); // Ensure cleanup on connection error
        socket = null;
      });

      // Listen for incoming incident data
      socket.on('new_detection', (detection: AiDetection) => {
        console.log('[Socket Middleware] Received new detection:', detection);
        // Ensure data format matches AiDetection before dispatching
      ...
      // Optional: Handle other custom events from the server
      // socket.on('some_other_event', (data) => { ... });
    ...
    default:
      // Allow other actions to pass through
      return next(action);
  }
};
